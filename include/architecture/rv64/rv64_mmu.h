    ...
    private:
        bool attach(unsigned int lvl2, unsigned int lvl1, const Page_Table *pt, unsigned int n, RV64_Flags flags)
        {
            ...
            if (_pd->get_entry(lvl2))
            {
                ...
                Page_Directory *aux = new ((void *)(pte2phy(pd1->get_entry(lvl1)))) Page_Directory();
                db<MMU>(WRN) << hex << (((CPU::Reg64) aux) & ~0x1FFUL) << endl;
                db<MMU>(WRN) << *aux << endl;

                db<MMU>(WRN) << hex << (((CPU::Reg64) pt) & ~0x1FFUL) << endl;
                db<MMU>(WRN) << *pt << endl;
