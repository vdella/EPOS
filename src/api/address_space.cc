...
{
    db<Address_Space> (TRC) << "Address_Space(pd=" << pd << ") [Directory::pd=" << MMU::Directory::pd() << "] => " << this << endl;
}

Address_Space::Address_Space()
{
    db<Address_Space> (TRC) << "Address_Space() [Directory::pd=" << MMU::Directory::pd() << "] => " << this << endl;
}

Address_Space::~Address_Space()
{
    db<Address_Space>(TRC) << "~Address_Space(this=" << this << ") [Directory::pd=" << MMU::Directory::pd() << "]" << endl;
}

Address_Space::Log_Addr Address_Space::attach(Segment * seg)
{
    Log_Addr tmp = MMU::Directory::attach(*seg);
...
{
    Log_Addr tmp = MMU::Directory::attach(*seg, addr);
...
void Address_Space::detach(Segment * seg, Log_Addr addr)
{
    ...
    MMU::Directory::detach(*seg, addr);
}

Address_Space::Phy_Addr Address_Space::physical(Log_Addr address)
{
    return MMU::Directory::physical(address);
}
