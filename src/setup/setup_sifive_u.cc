...
void Setup::enable_paging()
{
    ...
    for (unsigned long i = 0; i < PD_ENTRIES_LVL_2; i++) {
        ...
        addr += PAGE_SIZE;
        pd_lv1->remap(addr, RV64_Flags::PD, 0, PD_ENTRIES_LVL_1);
...
void Setup::init_mmu()
{
    ...
    unsigned int lvl2 = (qtt_of_pages_lv2) > PT_ENTRIES ? PT_ENTRIES : qtt_of_pages_lv2;
    unsigned int lvl1 = (qtt_of_pages) > PT_ENTRIES ? PT_ENTRIES : (qtt_of_pages);
    unsigned int lvl0 = pages > PT_ENTRIES ? PT_ENTRIES : pages;
    ...
    addr += PAGE;
    _master->remap(addr, RV64_Flags::V, 0, lvl2);
    ...
    for (unsigned i = 0; i < lvl2; i++)
    {
        ...
        pd_lvl1->remap(addr_lvl1, RV64_Flags::V);
        addr += PAGE;
        addr_lvl1 += lvl1 * PAGE;
    }
    ...
    for (unsigned i = 0; i < lvl2; i++)
    {
        ...
        for (unsigned j = 0; j < lvl1; j++)
        {
            ...
            pt->remap(addr_lvl0, RV64_Flags::SYS);
            addr += PAGE;
            addr_lvl0 += lvl0 * PAGE;
        }
    }
